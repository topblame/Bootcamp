...
public class OpenAIKeywordSummary {
	...
	//딥러닝 모델: OpenAI의 GPT-4o 모델->Transformer 기반 대규모 언어 모델
    //OpenAI Chat Completions API(비지도 학습): 챗 형식 응답을 반환하는 API(예: 뉴스분석)
	public static void main(String[] args) {
		Scanner scanner = new Scanner(System.in);
		//사용자가 여러줄의 문서를 입력할 수 있게.
		StringBuilder inputText = new StringBuilder();
		System.out.println("텍스트를 입력하세요 (텍스트 종료 END) 예: 문서 이메일 기사 등");
		while(true) {
			String line = scanner.nextLine();
			if(line.equalsIgnoreCase("EEND")) {
				break;
			}
			inputText.append(line).append("\n");	
		}
		String userText=inputText.toString();
        System.out.println("입력받는 텍스트:");
        System.out.println(userText);
        try {
			String response = getKeywordSummary(userText);
			System.out.println("키워드 및 요약");
	...
	public static String getKeywordSummary(String text) throws Exception { 
      //요청 데이터 생성
      Map<String, Object> requestData = Map.of(
                "model", "gpt-4o",
                "messages", List.of(
                    Map.of("role", "user", "content", "다음 텍스트에서 키워드를 추출하고 간단한 요약을 제공해주세요 "+text)
                ),
                "max_tokens", 500,
                "temperature", 0.7
            );
